...
- Проверка изоляции reviewer fixtures 2026-08-21T06:32:55Z сначала воспроизвела отсутствие отдельных credential/network dependencies, затем прошла с маркерами `fixture-isolation-pass` и `fixture-release-isolation-pass`: любой явный Debug `YP_SCREENSHOT_STATE` использует отдельные учётные данные в памяти и сессию, которая локально отклоняет все сетевые запросы; обычный Debug сохраняет прежние Keychain/backend dependencies, а Release игнорирует screenshot state.
- После исправления свежие Debug и Release generic iOS Simulator builds завершились с exit code 0; Release снова создал app, widget и оба notification extensions. Поиск в свежем Release app не нашёл `YP_SCREENSHOT_STATE`, Debug-only credential, fixture URL protocol или fixture identities. Фокусные backend deletion/auth tests дали `5 passed`, полный backend — `146 passed, 1 warning`; Ruff check/format остались чистыми.
