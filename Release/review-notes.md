...
Deployment order is YDB schema v2 before backend code and API Gateway traffic. Universal sharing remains a manual review blocker until that order is followed, the gateway is deployed, AASA is verified directly in production, and all six checks in `Release/manual-device-checks.md` pass on physical iPhones. No production or physical-device result is claimed by these notes.
