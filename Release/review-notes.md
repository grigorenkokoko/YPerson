...
Private-phone path: save a phone on the issuer, enable `Поделиться телефоном · Face ID`, complete Face ID or device-passcode authentication, then prepare Bluetooth or Show short code. Only the confirmed recipient receives that phone as a directional connection grant; the reciprocal view, unrelated connections, stored public card, and every QR remain phone-free. Leaving Exchange clears the authorization. Physical-device Face ID/passcode, mutual Bluetooth, and two-installation manual claim are still pending and are not claimed by the local deployment-contract tests.
...
4. Face ID: Card → Open private fields, or enable `Поделиться телефоном · Face ID` in Exchange for the next outgoing Bluetooth/manual preparation. Device passcode and the phone-free public card/QR remain available.
...
Backend: public `GET /config` accepts no personal data, supports ETag/304, and can only disable disclosed features/analytics. It cannot add permissions, data collection, tracking, code, or secrets. `/health` and authenticated `/sync` provide health and combined profile/exchange/moderation sync. Sync v2 uses request `privateFields`/`exchangeCode` and response `exchangeCode`/`exchangeExpiresAt`; temporary private fields share the claim's maximum 10-minute TTL. Raw token/code is never stored: normalized credentials are SHA-256 digested.

Offline: the card and phone-free offline QR remain available; pending sync/report/deletion is retried. A real manual code requires network access and authenticated issuer/claimant installations; it is not a static offline fallback.

Known unfinished scope: recipient-specific private-audio persistence is not implemented. A private-phone grant retains the confirmed value until connection/profile deletion, but connection-level private-grant revoke/update and propagation of later phone edits are not implemented. These limitations, the physical-device checks, and the third-party vCard conformance blocker above remain open; this document does not make the build release-ready.
