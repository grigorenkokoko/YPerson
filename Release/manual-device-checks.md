...
- [ ] Bluetooth: на двух iPhone открыть экран обмена и запустить поиск сначала на A, затем на B; убедиться, что публичная карточка и токен отправляются серверу уже при запуске поиска, а в BLE рекламируется только эфемерный токен. Проверить, что окно поиска закрывается и подтверждение появляется на обоих. Повторить в порядке B → A: подтверждение должно только claim-ить peer token и сохранять связь; подтвердить на обоих и проверить сохранение обеих публичных карточек без телефона. Затем отменить один обмен и убедиться, что токен удаляется немедленно либо не позднее 10 минут по TTL.
...
Все пункты ниже имеют статус **PENDING**: симулятор не является evidence для взаимного публичного Bluetooth-обмена, Face ID или проверки поведения на двух физических устройствах.
...
- [ ] **PENDING — Bluetooth public-only:** на двух iPhone сначала авторизовать `Поделиться телефоном по коду · Face ID` только на A, затем завершить взаимный Bluetooth-обмен и проверить, что ни A, ни B не получает телефон, а обе карточки и QR остаются публичными. Убедиться, что public BLE не удалён и consent A по-прежнему относится только к следующему ручному коду. Private BLE не тестируется и не заявляется до реализации recipient-bound mutual pairing.
