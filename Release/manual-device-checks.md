...
Статус simulator-проверки оформления (2026-08-20): DEBUG-сборка для iPhone 14 Pro (iOS 16.4) собрана и запущена с `YP_SCREENSHOT_STATE=S6`, но до маршрута S6 приложение показало «YPerson не запущен — Не удалось получить защищённые данные установки». Поэтому preview, переключение всех четырёх палитр, VoiceOver selected state, Dynamic Type, отмена редактора, сохранение выбора и изображение сохранённой/полученной карточки не подтверждены симулятором и остаются ручными проверками ниже.
...
- [ ] Оформление: проверить все четыре шаблона (`standard-clean`, `standard-contrast`, `mint-conference`, `indigo-studio`) в светлой и тёмной теме, live preview, checkmark/subtitle и VoiceOver state выбранного оформления.
- [ ] Оформление: для каждого шаблона сохранить карточку, перезапустить приложение и подтвердить сохранение `templateID`; проверить оформление у полученной карточки и в экспортированном изображении.
- [ ] Оформление и ATT: при denied ATT убедиться, что все четыре шаблона по-прежнему доступны и выбранное оформление не меняется.
...
- [ ] Геопозиция When In Use: перед обменом включить сохранение места и проверить точную, approximate, denied и restricted геопозицию; завершённый обмен должен прикрепить человекочитаемую подпись только к полученной карточке, отменённый — ничего не сохранять. Проверить ручной ввод, возможность добавить место после обмена и отсутствие координат в `/sync` и AppMetrica.
