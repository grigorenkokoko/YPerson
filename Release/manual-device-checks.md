...
- [ ] Bluetooth: на двух iPhone открыть экран обмена и запустить поиск сначала на A, затем на B; убедиться, что backend preparation публичной карточки и токена начинается до discovery, а в BLE рекламируется только эфемерный токен. Проверить, что окно поиска закрывается и локальное подтверждение появляется на обоих. Повторить в порядке B → A: каждая installation должна независимо claim-ить peer token и сохранять свою связь только после собственного локального подтверждения; отдельного peer-confirmation signal нет. Проверить сохранение обеих публичных карточек без телефона. Затем отменить один обмен и убедиться, что токен удаляется немедленно либо не позднее 10 минут по TTL.
...
## Честный обмен телефоном и коротким кодом

Все пункты ниже имеют статус **PENDING**: симулятор не является evidence для двустороннего публичного Bluetooth-обмена с независимыми локальными claims, Face ID или проверки поведения на двух физических устройствах.

- [ ] **PENDING — два устройства:** на iPhone A показать `YP-XXXX-XXXX-XXXX`, на iPhone B ввести тот же код с префиксом и без него, с пробелами и разделителями; подтвердить одну успешную выдачу, сохранение карточки и отказ при повторном claim без раскрытия причины.
- [ ] **PENDING — Bluetooth public-only:** на двух iPhone сначала авторизовать `Поделиться телефоном по коду · Face ID` только на A, затем на каждом iPhone независимо подтвердить локальный claim двустороннего Bluetooth-обмена и проверить, что ни A, ни B не получает телефон, а обе карточки и QR остаются публичными. Не ожидать peer-confirmation signal. Убедиться, что public BLE не удалён и consent A по-прежнему относится только к следующему ручному коду. Private BLE не тестируется и не заявляется до реализации recipient-bound mutual pairing.
- [ ] **PENDING — Face ID / код-пароль single-use:** отдельно проверить успешный Face ID, отмену Face ID и успешный fallback на код-пароль; switch включается только после успеха. При нажатии `Показать короткий код` он сразу выключается до ответа сети. Двойной tap не должен запустить два private prepare, а ошибка или отмена preparation должна требовать новой аутентификации перед следующей передачей телефона.
- [ ] **PENDING — expiry:** дождаться server expiry показанного кода и проверить generic `Код не подтверждён`, отсутствие повторного claim и отсутствие телефона у неподтверждённого получателя.
- [ ] **PENDING — background:** свернуть приложение на экране кода и во время публичного Bluetooth-поиска, затем вернуться; проверить отсутствие крэша, корректную отмену/expiry credential и сброс неиспользованного разрешения на передачу телефона после ухода с Exchange.
- [ ] **PENDING — non-durable credentials:** прервать сеть во время manual claim и best-effort cancel, перезапустить приложение и подтвердить отсутствие автоматической повторной отправки raw token/code; пользовательский retry требует нового действия, а несекретные publish/report/delete операции продолжают stable retry.
- [ ] **PENDING — clipboard:** нажать `Скопировать код` и проверить, что буфер содержит только канонический короткий код, без имени, телефона и payload визитки.
- [ ] **PENDING — VoiceOver:** проверить порядок фокуса, название switch `Поделиться телефоном по коду · Face ID`, пояснение публичных QR/Bluetooth, label `Короткий код обмена: …` и announcement после копирования.
- [ ] **PENDING — Dynamic Type:** проверить Exchange и экран короткого кода до Accessibility Extra Extra Extra Large без обрезки кода, expiry, статуса и кнопок.
