...
Все пункты ниже имеют статус **PENDING**: симулятор не является evidence для взаимного Bluetooth-обмена, Face ID или проверки поведения на двух физических устройствах.
...
- [ ] **PENDING — Bluetooth private sharing:** на двух iPhone включить передачу телефона только на A, завершить взаимное подтверждение и проверить, что B получает телефон A, A не получает телефон B, а QR обеих сторон остаются публичными.
- [ ] **PENDING — Face ID / код-пароль:** отдельно проверить успешный Face ID, отмену Face ID и успешный fallback на код-пароль; switch включается только после успеха и остаётся выключенным после отмены.
...
- [ ] **PENDING — background:** свернуть приложение на экране кода и во время Bluetooth-поиска, затем вернуться; проверить отсутствие крэша, корректную отмену/expiry credential и сброс разрешения на передачу телефона после ухода с Exchange.
...
- [ ] **PENDING — VoiceOver:** проверить порядок фокуса, название switch, пояснение публичного QR, label `Короткий код обмена: …` и announcement после копирования.
