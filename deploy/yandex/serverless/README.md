...
Обязательный порядок: YDB schema v2 before backend code and API Gateway traffic.
Сначала примените схему v2, затем выкатывайте использующую её ревизию и только
после этого направляйте gateway traffic. Пока эти шаги не выполнены в указанном
порядке, production AASA, Universal Link, vCard, reply и revoke проверки
остаются блокерами и не могут считаться пройденными по локальной сборке.
